...
public class GreedyMesh
{
    ...
    // C# port: https://gist.github.com/Vercidium/a3002bd083cce2bc854c9ff8f0118d33
            ...
            for (pos[axis] = -1; pos[axis] < chunkSize;)
            {
                ...
                for (pos[vAxis] = 0; pos[vAxis] < chunkSize; ++pos[vAxis])
                {
                    for (pos[uAxis] = 0; pos[uAxis] < chunkSize; ++pos[uAxis])
                    {
                        ...
                        if (currentBlock.IsOpen && !compareBlock.IsOpen && pos[axis] >= 0)
                        {
                            mask[n++] = FaceType.FromVoxel(currentBlock, direction, false);
                        }
                        else if (!currentBlock.IsOpen && compareBlock.IsOpen && pos[axis] < chunkSize - 1)
                        {
